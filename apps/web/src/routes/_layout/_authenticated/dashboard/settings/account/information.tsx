...
function RouteComponent() {
  ...
  const [accounts, setAccounts] = useState<Array<{ provider: string }> | null>(null);
  const [isLinkingGithub, setIsLinkingGithub] = useState(false);

  useEffect(() => {
    authClient.listAccounts().then((res) => {
      if ((res as any).data) setAccounts((res as any).data);
    });
  }, []);
  ...
  const handleLinkGithub = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : t("settings:informationPage.updateError"));
    } finally {
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {isGithubLinked ? user?.email : t("settings:informationPage.linkedAccountsSubtitle")}
                  </p>
                </div>
              </div>
              {isGithubLinked ? (
                ...
                <Button variant="outline" size="sm" onClick={handleLinkGithub} disabled={isLinkingGithub}>
                  <GithubIcon className="h-4 w-4 mr-2" />
                  {isLinkingGithub ? t("settings:informationPage.linking") : t("settings:informationPage.linkGithub")}
                </Button>
              )}
            </div>
          </div>
