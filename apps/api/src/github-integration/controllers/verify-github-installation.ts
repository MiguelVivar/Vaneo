...
async function verifyGithubInstallation({
  ...
  // Vaneo custom: try user OAuth first for user-specific repos
  if (userId) {
    ...
    if (userOctokit) {
      try {
        ...
        return {
          ...
          permissions: (repo.permissions as unknown as Record<string, string>) ?? null,
        ...
        if (githubError.status === 404) {
          // fall through to App check — repo not found via OAuth, try App
        } else if (githubError.status !== 404) {
          // For non-404 OAuth errors, still fall through to App for resilience
        }
      }
    }
  }
